module.exports = function(RED) {
  ...
  function GraphqlExecNode(config) {
    ...
    function doLogin() {
      node.status({
        fill: "blue",
        shape: "ring",
        text: RED._("graphql-timeout.status.connecting")
      });
      ...
      request(
        ...
        function(error, response, body) {
          if (response) {
            switch (response.statusCode) {
              case 200:
                node.status({
                  fill: "green",
                  shape: "dot",
                  text: RED._("graphql-timeout.status.connected")
                });
                ...
                if (response.body.response.errorCode) {
                  ...
                  switch (response.body.response.errorCode) {
                    case "RBAC": // token not recognized
                      errorMsg = RED._("graphql-timeout.errors.tokenExpLogin");
                      node.msg.payload = {
                    ...
                    case "INVALID_CREDENTIALS": // token not recognized
                      errorMsg = RED._("graphql-tiemout.errors.badCreds");
                    ...
                    default:
                      // other issue
                      errorMsg =
                        RED._("graphql-timeout.errors.error401") +
                        " " +
              ...
              case 403: // bad url, api version number
                errorMsg = RED._("graphql-timeout.errors.badRest");
                node.status({ fill: "red", shape: "dot", text: errorMsg });
              ...
              case 500: // bad credentials
                ...
                errorMsg = RED._("graphql-timeout.errors.badCreds");
                node.status({ fill: "red", shape: "dot", text: errorMsg });
                ...
                if (response) {
                  ...
                  if (responseJSON && responseJSON.response.message) {
                    ...
                    node.status({
                      fill: "red",
                      shape: "dot",
                      text: RED._("graphql-timeout.status.failedConn") + status
                    });
                  }
                } // if response
                node.msg.payload = response;
            ...
            switch (error.code) {
              case "ENOTFOUND":
                errorMsg = RED._("graphql-timeout.errors.server");
                node.status({ fill: "red", shape: "dot", text: errorMsg });
                node.error(errorMsg, node.msg);
                break;
              case "ETIMEDOUT":
                errorMsg = RED._("graphql-tiemout.errors.timeout");
                node.status({ fill: "red", shape: "dot", text: errorMsg });
                node.error(errorMsg, node.msg);
                break;
              default:
                node.status({
                  fill: "red",
                  shape: "dot",
                  text: RED._("graphql-timeout.status.failed") + " " + error.code
                });
    ...
    function callGraphQLServer(query, variables = {}, customHeaders = {}, timeout) {
          ...
          switch (true) {
            case response.status == 200 && !response.data.errors:
              node.status({
                fill: "green",
                shape: "dot",
                text: RED._("graphql-timeout.status.success")
              });
            ...
            case response.status == 200 && !!response.data.errors:
              node.status({
                fill: "yellow",
                shape: "dot",
                text: RED._("graphql-timeout.status.gqlError")
              });
    ...
    node.on("input", function(msg) {
      ...
      node.timeout = msg.timeout || node.timeout;
      ...
      if (!config.token) {
        ...
        callGraphQLServer(query, variables, node.customHeaders, node.timeout);
  ...
  RED.nodes.registerType("graphql-timeout", GraphqlExecNode);
};
